...
void bot_ai::_calculatePos(Position& pos) const
{
    ...
    float size = bmover->GetCombatReach()/3.f;
    if (bmover->CanFly())
    {
...
void bot_ai::BotMovement(BotMovementType type, Position const* pos, Unit* target, bool generatePath) const
{
    ...
    Unit* mover = (veh && (veh->GetSeatForPassenger(me)->Flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)) ? veh->GetBase() : !veh ? me : nullptr;
    if (!mover)
        return;
...
bool bot_ai::CanBotMoveVehicle() const
{
    return me->GetVehicle() &&
        (me->GetVehicle()->GetSeatForPassenger(me)->Flags & VEHICLE_SEAT_FLAG_CAN_CONTROL);
}
...
{
    return !me->GetVehicle() ? true : (me->GetVehicle()->GetSeatForPassenger(me)->Flags & VEHICLE_SEAT_FLAG_CAN_ATTACK);
}
//GETVEHICLETARGET
...
void bot_ai::CalculateAttackPos(Unit* target, Position& pos) const
{
    ...
    if ((target->m_movementInfo.GetMovementFlags() & MOVEMENTFLAG_FORWARD) && target->HasInArc(M_PI/2, me))
    ...
    if (me->GetVehicle())
    {
        ...
        for (uint8 i = 0; i < 4; ++i)
        {
            ...
            if (master->GetDistance(ppos) < (followdist > 30.f ? 60.f : followdist < 10 ? 20.f : float(followdist*2)))
                break;
    ...
    for (uint8 i = 0; i < 5; ++i)
    {
        ...
        bool toofaraway = master->GetDistance(/*x,y,z*/ppos) > (followdist > 38.f ? 38.f : followdist < 20 ? 20.f : float(followdist));
        bool outoflos = !target->IsWithinLOS(/*x,y,z*/ppos.m_positionX, ppos.m_positionY, ppos.m_positionZ);
...
void bot_ai::GetInPosition(bool force, Unit* newtarget, Position* mypos)
{
    ...
    if (HasRole(BOT_ROLE_RANGED) || HasVehicleRoleOverride(BOT_ROLE_RANGED))
    {
        ...
        if (mover->GetExactDist2d(&attackpos) > 4.f || !mover->IsWithinLOSInMap(newtarget))
        {
            BotMovement(BOT_MOVE_POINT, &attackpos);
            //me->GetMotionMaster()->MovePoint(newtarget->GetMapId(), attackpos);
    ...
    {
        Vehicle* veh = me->GetVehicle();
        Unit* mover = CanBotMoveVehicle() ? veh->GetBase() : !veh ? me : nullptr;
        if (mover && !JumpingOrFalling() && ((!mover->HasUnitState(UNIT_STATE_CHASE) && !mover->isMoving()) || (!mover->HasUnitState(UNIT_STATE_CHASE_MOVE) && mover->GetDistance(newtarget) > 1.5f)))
...
void bot_ai::DoEmeraldDrakeVehicleStrats(uint32 diff)
{
    ...
    if (IsCasting(drake))
    {
        ...
        if (drakeHpPct <= 30)
            interrupt = true;
        else if (mmover->isMoving() && drake->GetDistance(mmover) >= 75.f && !mmover->HasInArc(M_PI / 2, drake))
            interrupt = true;
...
void bot_ai::DoAmberDrakeVehicleStrats(uint32 diff)
{
    ...
    if (IsCasting(drake))
    {
        ...
        if (mmover->isMoving() && drake->GetDistance(mmover) >= 60.f && !mmover->HasInArc(M_PI / 2, drake))
            interrupt = true;
    ...
    if (checkAurasTimer <= lastdiff)
    {
            ...
            !mover->isMoving() && !IsCasting(mover) && !IsShootingWand(mover))
        {
            ...
            else
            {
                ...
                if (mover->GetExactDist2d(&attackpos) > 4.f || !mover->IsWithinLOSInMap(victim))
...
void bot_ai::AfterBotOwnerEnterVehicle()
{
    if (!me->GetVehicle() && master->GetVehicleCreatureBase() && master->GetVehicleCreatureBase()->m_spells[0] &&
        master->GetVehicle()->GetSeatForPassenger(master)->CanEnterOrExit() && master->GetVehicleBase()->IsControlledByPlayer())
    {
        uint32 creEntry = 0;
        uint32 vehEntry;
...
void bot_ai::ChooseVehicleForEncounter(Player const* owner, uint32 &creEntry, uint32 &vehEntry) const
{
    ...
    switch (mVeh->GetBase()->GetEntry())
    {
        ...
        //    break;
        default:
            if (mVeh->GetSeatForPassenger(owner)->Flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
            {
